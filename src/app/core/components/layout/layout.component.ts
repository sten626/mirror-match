...
export class LayoutComponent implements OnDestroy, OnInit {
  ...
  constructor(
    ...
    this.hasDraftStarted$ = this.store.select(fromRoot.hasDraftStarted);
  ...
  ngOnInit() {
    this.urlSubscription = this.store
      .select(fromRoot.getUrl)
      .subscribe((route) => {
        if (route && route.includes('/')) {
          const routeSection = route.split('/')[1];
          this.toolbarHeader =
            routeSection[0].toUpperCase() + routeSection.slice(1);
        } else {
          this.toolbarHeader = '';
        }
      });
